...
The ASCC (Assembly Screen for Cobionts and Contaminants) pipeline is designed to run multiple decontamination tools and then merge their results. One of the main outputs of the pipeline is a CSV (comma-separated values) file where the rows correspond to sequences in the assembly and the columns contain the classification results from the constituent tools.
