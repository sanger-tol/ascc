...
## Inputs
The only required input is an assembly FASTA file. The pipeline can take reads (both long and short reads) as an input and map them with minimap2 (Li 2018) to determine coverage. It can also take organellar FASTA files as input to use in the detection of organellar contamination in a nuclear genome assembly (using BLAST). Version 1 of the ASCC pipeline will take a singular samplesheet.csv, containing the assembly name, type and file path. (SHOW IN IMAGE) as well as a params.yaml which contains the arguments and file locations for tools contained in the pipeline.
