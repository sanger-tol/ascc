...
def parse_args(argv = None):
    ...
    parser.add_argument(
        ...
        type = str,
        default = "npy.csv",
        help = "The output CSV file naming"
    )
...
def return_canonical_only(kmer_list):
    ...
    kmers not found in its reverse compliment
    """
    return kmer_list[:(int(len(kmer_list)/2))]


def get_compliment(kmer) -> str:
    """
    Generate the compliment of a given sequence
    """
    compliment_dict = { 'A':'T', 'T':'A', 'C':'G', 'G':'C' }
    return ''.join([compliment_dict[i] for i in kmer])


def compliment_check(kmer_list) -> None:
    """
    Ensure that there are no complementary kmers found in the
    list.
    """
    for i in kmer_list:
        comp_i = get_compliment(i)
...
def main() -> None:
    ...
    # Validate kmer list
    compliment_check(canonical_kmers)
