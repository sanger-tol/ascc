...
def main(args):
    if os.path.isdir(args.main_btk_datasets) is False:
        sys.stderr.write(
            f"The BlobToolKit dataset ({args.main_btk_datasets}) was not found!\n"
        )
        sys.exit(1)
    ...
    main_btk_dataset_files = [
        f
        for f in os.listdir(args.main_btk_datasets)
        if os.path.isfile(os.path.join(args.main_btk_datasets, f))
    ]
    main_btk_dataset_files = [
        f for f in main_btk_dataset_files if f not in not_copying_list
    ]
    for main_btk_dataset_file in main_btk_dataset_files:
        main_btk_dataset_file_full_path = (
            f"{args.main_btk_datasets}/{main_btk_dataset_file}"
        )
        copied_file_full_path = os.path.abspath(
            f"{args.new_output_directory}/{main_btk_dataset_file}"
        )
        shutil.copy(main_btk_dataset_file_full_path, copied_file_full_path)

    btk_busco_files = [
        f
        for f in os.listdir(args.btk_busco_datasets)
        if os.path.isfile(os.path.join(args.btk_busco_datasets, f))
    ]
    for btk_busco_file in btk_busco_files:
        btk_busco_file_full_path = f"{args.btk_busco_datasets}/{btk_busco_file}"
        copied_file_full_path = os.path.abspath(
            f"{args.new_output_directory}/{btk_busco_file}"
        )
        shutil.copy(btk_busco_file_full_path, copied_file_full_path)

    merged_jsons_dict = create_meta_json_contents(
        args.main_btk_datasets, args.btk_busco_datasets
    )
    ...
    btk_busco_table_outpath = (
        f"{args.new_output_directory}/btk_busco_summary_table_full.tsv"
    )
