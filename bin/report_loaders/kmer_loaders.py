...
def load_kmer_dim_reduction_results(dir_path):
    ...
    """
    print(f"Loading k-mer dimensionality reduction results from: {dir_path}", file=sys.stderr)
    ...
    try:
        ...
        for item in os.listdir(dir_path):
            ...
            if os.path.islink(item_path):
                try:
                    ...
                    print(f"Warning: Could not resolve symlink {item_path}: {e}", file=sys.stderr)
        ...
        if not method_dirs:
            ...
            dir_basename = os.path.basename(os.path.realpath(dir_path)) # Resolve potential link
            if "_kmers_dim_reduction_dir" in dir_basename:
                 method_dirs = [dir_path]
            else:
                 print(f"No directories matching '*_kmers_dim_reduction_dir' found in {dir_path}", file=sys.stderr)
                 return None # Explicitly return None if no valid dirs found

        print(f"Found {len(method_dirs)} dimensionality reduction method sources (dirs/links)", file=sys.stderr)

        for method_source_path in method_dirs: # Iterate through the paths found (could be dirs or links)
            ...
            try:
                ...
                if not os.path.isdir(method_dir_actual):
                    print(f"Error: Resolved path {method_dir_actual} is not a directory. Skipping {method_source_path}.", file=sys.stderr)
                    continue
            except OSError as e:
                print(f"Error: Could not resolve path {method_source_path}: {e}. Skipping.", file=sys.stderr)
                continue
            ...
            print(f"Processing method: {method_name} from source {method_source_path} (actual: {method_dir_actual})", file=sys.stderr)
            ...
            clustering_files = [f for f in os.listdir(method_dir_actual) if f.endswith("_clustering_metrics.txt")]
            for clustering_file in clustering_files:
                try:
                    with open(os.path.join(method_dir_actual, clustering_file), "r") as f:
            ...
            viz_files = [f for f in os.listdir(method_dir_actual) if f.endswith("_visualisation.png")]
