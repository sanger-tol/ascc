...
def load_fcsgx_results(file_path):
    ...
    try:
        ...
        if not data.strip() or data.strip() == "accessionlengthactionrangename":
            print(f"FCS-GX file contains only whitespace or header: {file_path}", file=sys.stderr)
            return "FCS-GX check was run, but no issues were found."
        # Add debug output
        print(f"Successfully loaded FCS-GX data from {file_path}, content length: {len(data)}", file=sys.stderr)
...
def load_fcsgx_report_as_table(file_path):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("##"):
                metadata_raw = line
                print(f"Found FCS-GX metadata at line {i}: {metadata_raw[:50]}...", file=sys.stderr)
        ...
        metadata_table = None
        if metadata_raw:
            metadata_table = format_fcsgx_metadata(metadata_raw, "fcsgx_report_metadata_table")
            if metadata_table:
                print(f"Successfully formatted FCS-GX report metadata as table", file=sys.stderr)
            else:
                print(f"Failed to format FCS-GX report metadata as table, will use raw text", file=sys.stderr)
        ...
        if header_line:
            try:
                ...
                print(f"Error converting FCS-GX report data to DataFrame: {e}", file=sys.stderr)
