...
def load_contamination_check_merged_table(file_path):
    ...
    try:
        ...
        table_html = df.to_html(
            ...
            index=False,
            table_id="cobiont_check_merged_table",
        )
...
def load_phylum_coverage_data(file_path):
    ...
    try:
        ...
        table_html = df.to_html(
            classes="table table-striped", index=False, table_id="phylum_coverage_table"
        )
        ...
        print(
            f"Successfully loaded and formatted phylum coverage data from {file_path}",
            file=sys.stderr,
        )
    ...
    try:
        ...
        if warning_rows:
            ...
            if non_warning_comments:
                comments_html += (
                    "<p><strong>Other Comments:</strong></p><pre><code>"
                    + "\n".join(non_warning_comments)
                    + "</code></pre>"
                )
        elif comments:
            # If no warnings were parsed but there are comments, show them as plain text
            comments_html = (
                "<p><strong>Comments/Warnings:</strong></p><pre><code>"
                + "\n".join(comments)
                + "</code></pre>"
            )
        ...
        """
        print(
            f"Successfully loaded and formatted Trailing Ns data from {file_path}",
            file=sys.stderr,
        )
        return wrapped_table
    except Exception as e:
        ...
        if comment.startswith("WARNING:"):
            ...
            if len(parts) >= 5 and "Ns of total" in comment:
                try:
                    ...
                    print(
                        f"Error parsing warning comment: {comment} - {e}",
                        file=sys.stderr,
                    )
...
def format_trim_ns_warnings_table(warning_rows):
    ...
    try:
        ...
        warning_table_html = warning_df.to_html(
            classes="table table-striped",
            index=False,
        ...
        warnings_text = "\n".join(
            [
                f"{row['Sequence ID']}: {row['Percentage of Ns']} of {row['Total Length']}"
                for row in warning_rows
            ]
        )
        return (
            f"<p><strong>Warnings:</strong></p><pre><code>{warnings_text}</code></pre>"
        )
...
def load_vecscreen_results_as_table(file_path):
    ...
    try:
        ...
        table_html = df.to_html(
            classes="table table-striped",
            index=False,
            table_id="vecscreen_plain_table",
            escape=False,
        )
        ...
        print(
            f"Successfully loaded and formatted VecScreen data from {file_path} with {len(data_rows)} rows",
            file=sys.stderr,
        )
        return wrapped_table
    except Exception as e:
...
def load_fcs_adaptor_results_as_table(file_path):
    ...
    if os.path.getsize(file_path) == 0:
        ...
        return '<div class="success-container">FCS-adaptor check was run and no adaptor contamination was detected.</div>'
    ...
    try:
        ...
        table_html = df.to_html(
            classes="table table-striped",
            index=False,
            table_id="fcs_adaptor_plain_table",
            escape=False,
        )
        ...
        print(
            f"Successfully loaded and formatted FCS-adaptor data from {file_path}",
            file=sys.stderr,
        )
        return wrapped_table
    except Exception as e:
...
def load_autofiltering_results_as_table(file_path):
    ...
    try:
        ...
        try:
            ...
            print(
                f"CSV parsing error, attempting to fix column mismatch: {e}",
                file=sys.stderr,
            )
            ...
            if header_cols != data_cols:
                print(
                    f"Column mismatch detected: header has {header_cols} columns, data has {data_cols} columns",
                    file=sys.stderr,
                )
        ...
        table_html = df.to_html(
            classes="table table-striped",
            index=False,
            table_id="autofiltering_plain_table",
            escape=False,
        )
        ...
        print(
            f"Successfully loaded and formatted autofiltering data from {file_path}",
            file=sys.stderr,
        )
        return wrapped_table
    except Exception as e:
        print(
            f"Error loading or formatting autofiltering results: {e}", file=sys.stderr
        )
