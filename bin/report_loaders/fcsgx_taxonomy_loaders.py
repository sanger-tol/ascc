...
def load_fcsgx_taxonomy_as_table(file_path):
    ...
    try:
        ...
        with open(file_path, 'r') as f:
            ...
            for i, line in enumerate(f):
                ...
                if "GX taxonomy analysis report" in line:
                    ...
                    print(f"Found taxonomy metadata at line {i}: {metadata_raw[:50]}...", file=sys.stderr)
                elif line.startswith("seq-id") or line.startswith("#seq-id"):
                    header_line = line.strip()
                    if header_line.startswith("#"):
                        header_line = header_line[1:]  # Remove leading # if present
                    print(f"Found taxonomy header at line {i}: {header_line}", file=sys.stderr)
                    break
        ...
        if metadata_raw:
            ...
            metadata_table = format_fcsgx_metadata(metadata_raw, "fcsgx_taxonomy_metadata_table")
            if metadata_table:
                print(f"Successfully formatted FCS-GX taxonomy metadata as table", file=sys.stderr)
            else:
                print(f"Failed to format FCS-GX taxonomy metadata as table, will use raw text", file=sys.stderr)
                metadata_table = metadata_raw
        
        if not header_line:
            print(f"Could not find header line in taxonomy file: {file_path}", file=sys.stderr)
        ...
        with tempfile.NamedTemporaryFile(mode='w+', delete=False) as temp_file:
            ...
            with open(file_path, 'r') as f:
                for line in f:
                    ...
                    if "GX taxonomy analysis report" in line or line.startswith("seq-id") or line.startswith("#seq-id"):
                        continue
        ...
        try:
            ...
            print(f"Falling back to simpler parsing approach for taxonomy file", file=sys.stderr)
            return fallback_load_fcsgx_taxonomy(file_path, metadata_table)
