...
def write_coverage_file(coverage_file_base_name, merged_bed_file, length_for_sequence, bedtools):
	...
	for sequence in coverage_for_sequence:
		percentage_coverage_for_sequence[sequence] = coverage_for_sequence[sequence] / length_for_sequence[sequence] * 100
	...
	for sequence in sorted(percentage_coverage_for_sequence.keys(), key = lambda x: percentage_coverage_for_sequence[x], reverse = True):
		...
		unfiltered_coverage_scaffold_handle.write(bed_with_coverage_format.format(sequence, 0, length_for_sequence[sequence], coverage_for_sequence[sequence], percentage_coverage_for_sequence[sequence]))
		if percentage_coverage_for_sequence[sequence] >= coverage_threshold:
			filtered_coverage_scaffold_handle.write(bed_with_coverage_format.format(sequence, 0, length_for_sequence[sequence], coverage_for_sequence[sequence], percentage_coverage_for_sequence[sequence]))
	...
	for sequence in merged_coord_list_for_sequence:
		for coord_pair in merged_coord_list_for_sequence[sequence]:
			unfiltered_coverage_region_handle.write( bed_with_coverage_format.format(sequence, str(coord_pair[0]-1), str(coord_pair[1]), coverage_for_sequence[sequence], percentage_coverage_for_sequence[sequence]) )
			if percentage_coverage_for_sequence[sequence] >= coverage_threshold:
				filtered_coverage_region_handle.write( bed_with_coverage_format.format(sequence, str(coord_pair[0]-1), str(coord_pair[1]), coverage_for_sequence[sequence], percentage_coverage_for_sequence[sequence]) )
