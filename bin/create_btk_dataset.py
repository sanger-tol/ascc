...
def add_custom_variables_to_btk_dataset(pipeline_run_folder, btk_dataset_folder):
    ...
    if os.path.isdir(pipeline_output_folder) == False:
        sys.stderr.write(
            "The directory for the output tables of the pipeline ({}) was not found\n".format(pipeline_output_folder)
        )
        sys.exit(1)
    ...
    if os.path.isfile(tiara_raw_output_path) and os.stat(tiara_raw_output_path).st_size > 0:
        ...
        add_tiara_command = 'blobtools add --text {} --text-delimiter "\t" --text-cols "identifier=identifiers,tiara=tiara" --text-header {}'.format(
            tiara_reformatted_output_path, btk_dataset_folder
        )
    ...
    if os.path.isfile(kmers_dim_reduction_output_path) and os.stat(kmers_dim_reduction_output_path).st_size > 0:
        ...
        for dim_reduction_method in used_dim_reduction_methods:
            add_embedding_command = 'blobtools add --text {path} --text-delimiter "," --text-cols scaff=identifiers,embedding_x_{dim_reduction_method}=embedding_x_{dim_reduction_method},embedding_y_{dim_reduction_method}=embedding_y_{dim_reduction_method} --text-header {btk_dataset_folder}'.format(
                path=kmers_dim_reduction_output_path,
                dim_reduction_method=dim_reduction_method,
                btk_dataset_folder=btk_dataset_folder,
            )
    ...
    if os.path.isfile(kraken_lineage_path) and os.stat(kraken_lineage_path).st_size > 0:
        for taxonomy_level in ("species", "genus", "family", "order", "class", "phylum", "kingdom", "domain"):
            add_kraken_command = 'blobtools add --text {} --text-delimiter "," --text-cols scaff=identifiers,nt_kraken_{}=nt_kraken_{} --text-header {}'.format(
                kraken_lineage_path, taxonomy_level, taxonomy_level, btk_dataset_folder
            )
    ...
    if os.path.isfile(fcs_gx_output_path) and os.stat(fcs_gx_output_path).st_size > 0:
        add_fcs_gx_results_command = 'blobtools add --text {} --text-delimiter "," --text-cols "scaff=identifiers,fcs_gx_top_tax_name=fcs_gx_top_tax_name,fcs_gx_div=fcs_gx_div,fcs_gx_action=fcs_gx_action" --text-header {}'.format(
            fcs_gx_output_path, btk_dataset_folder
        )
    ...
    if os.path.isfile(cobiontid_markerscan_output_path) and os.stat(cobiontid_markerscan_output_path).st_size > 0:
        add_cobiontid_markerscan_results_command = 'blobtools add --text {} --text-delimiter "," --text-cols "scaff=identifiers,CobiontID_MarkerScan_embl_ebi_ena=CobiontID_MarkerScan_embl_ebi_ena,CobiontID_MarkerScan_slv=CobiontID_MarkerScan_slv,CobiontID_MarkerScan_Cluster=CobiontID_MarkerScan_Cluster" --text-header {}'.format(
            cobiontid_markerscan_output_path, btk_dataset_folder
        )
...
def main(
    ...
    taxid,
    blastn_hits_path,
    uniprot_diamond_hits_path,
    nr_diamond_hits_path,
    mapped_reads_path,
    taxdump_path,
    threads,
    assembly_alias,
    ...
    blobtools_create_command = "blobtools create --fasta {} --meta {} --taxid {} --taxdump {} {}".format(
        assembly_fasta_path, assembly_yaml_path, taxid, taxdump_path, dataset_folder
    )
    ...
    export_table_command = "blobtools filter --table {}/collected_tables/btk_summary_table_full.tsv {}".format(
        pipeline_run_folder, dataset_folder
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tn", "--taxon_name", type=str, help="Taxon name")
    parser.add_argument("-ti", "--taxid", type=int, help="taxid")
    ...
    parser.add_argument(
        "mapped_reads_path", type=str, help="Path to the BAM file with mapped reads for coverage estimation"
    )
    parser.add_argument("-td", "--taxdump_path", type=str, help="Path to the directory with NCBI taxdump files")
    ...
    main(
        args.assembly_fasta_path,
        args.dataset_folder,
        args.pipeline_run_folder,
        args.assembly_title,
        args.taxon_name,
        args.taxid,
        args.blastn_hits_path,
        args.uniprot_diamond_hits_path,
        args.nr_diamond_hits_path,
        args.mapped_reads_path,
        args.taxdump_path,
        args.threads,
        args.assembly_alias,
        args.dry_run,
