...
def main(fasta_file, output_file):
    ...
    minslidingBase = (
        0.4  # maximum fraction of Ns in sliding window before alarm sets off
    )
    ...
    with open(fasta_file, "r") as fasta_input_handle:
        for record in SeqIO.parse(fasta_input_handle, "fasta"):
            ...
            if len(startseq) > 0 or len(endseq) > 0:
                if len(startseq) > 0:
                    output_handle.write(
                        "\t".join(["TRIM:", record.id, "1", str(len(startseq))])
                        + "\t\n"
                    )
                if len(endseq) > 0:
                    output_handle.write(
                        "\t".join(
                            [
                                "TRIM:",
                                record.id,
                                str(len(startseq) + realseq_length + 1),
                                str(len(seq_string)),
                            ]
                        )
                        + "\t\n"
            ...
            for seq_string_loop, seq_string_for_window in enumerate(
                ...
                if (
                    ...
                    for non_n_instance in non_n_iterator:
                        ...
                        non_n_regions.insert(
                            0, [current_non_n_start, current_non_n_end]
                        )
                        ...
                        for non_n_region in non_n_regions:
                            if non_n_region[1] >= current_non_n_end - winsize:
                                ...
                                if non_n_region[0] >= current_non_n_end - winsize:
                                    bases_in_window += (
                                        non_n_region[1] - non_n_region[0] + 1
                                    )
                                else:
                                    bases_in_window += (
                                        non_n_region[1] - non_n_region[0] + 1
                        ...
                        if bases_in_window >= minslidingBase * winsize:
                            ...
                            for i, non_n_region in enumerate(non_n_regions):
                                ...
                                if non_n_region[1] < non_n_regions[0][0] - winsize:
                                    ...
                                    current_window_start = max(
                                        non_n_region[0], non_n_regions[0][0] - winsize
                                    )
                                    ...
                                    bases_in_window_2 += (
                                        min(
                                            non_n_region[1],
                                            (current_window_start + winsize - 1),
                                        )
                            ...
                            if (
                                not (trackback_to_start_flag)
                                and start_of_first_base_block_in_window != 1
                            ):
