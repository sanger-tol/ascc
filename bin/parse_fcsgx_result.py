...
def get_fcs_gx_outfile_paths(fcs_gx_reports_folder):
    ...
    if len(taxonomy_files) != 1:
        sys.stderr.write("Error occurred when trying to find FCS-GX *.taxonomy.rpt file in {}\n".format(fcs_gx_reports_folder))
        sys.exit(1)
    ...
    if len(report_files) != 1:
        sys.stderr.write("Error occurred when trying to find FCS-GX *.fcs_gx_report.txt file in {}\n".format(fcs_gx_reports_folder))
        sys.exit(1)
...
def get_taxids_list(fcs_gx_taxonomy_file_path):
    ...
    if os.path.isfile(fcs_gx_taxonomy_file_path) is False:
        sys.stderr.write(f"The FCS-GX taxonomy file was not found at the expected location ({fcs_gx_taxonomy_file_path})\n")
        sys.exit(1)
    taxids_list = list()
...
def get_lineages_by_taxid(taxids_list, rankedlineage_path):
    ...
    """
    lineages_dict = dict()
    rankedlineage_col_names = ("taxid", "fcs_gx_name", "fcs_gx_species", "fcs_gx_genus", "fcs_gx_family", "fcs_gx_order", "fcs_gx_class", "fcs_gx_phylum", "fcs_gx_kingdom", "fcs_gx_domain")
...
def main(fcs_gx_reports_folder, ncbi_rankedlineage_path):
    ...
    for scaff, row_dict in collection_dict.items():
        out_line = "{},{},{},{},{},{},{},{},{}".format(scaff, row_dict["fcs_gx_top_tax_name"], row_dict["fcs_gx_top_taxid"], row_dict["fcs_gx_div"], row_dict["fcs_gx_coverage_by_div"], row_dict["fcs_gx_coverage_by_tax"], row_dict["fcs_gx_score"], row_dict["fcs_gx_multiple_divs_per_scaff"], row_dict["fcs_gx_action"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("fcs_gx_reports_folder", type=str, help="Path to directory with FCS-GX output files (*.taxonomy.rpt and *.fcs_gx_report.txt)")
